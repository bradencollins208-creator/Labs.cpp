...
class board3D: public board{
	public:
		...
		bool winCondition3D(string p){
			...
			//return false if no above condition is met
			return false;
		}//end winCondition()
};//end board3D
...
int main(){
	...
	//creates a board3D instance
	board3D newBoard;
	
	//creates score variables for the user and computer
	int userScore=0;
	int compScore=0;
	
	//creates a choice variable for the user
	int iChoice=1;
	
	//runs a while loop while until the user enters 0
	while(iChoice!=0){
		//creates a menu with options for the user
		cout<<"\nMenu"<<endl;
		...
		//checks that the choice was between 0 and 1
		...
		//runs if the users chooses 1
		if(iChoice==1){
			//outputs what the user and computer is
			cout<<"\nUser is:"<<user<<"\nComputer is:"<<computer<<endl;
			
			//outputs the scores for the user and computer
